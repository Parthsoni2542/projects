...
// $sql= "SELECT `cat_id` ,`category_name` FROM `category`";
  ...
  // "SELECT * FROM subcategory p , category c where p.cat_id = c.cat_id and p.sub_id";
  // "SELECT * FROM subcategory where cat_id =".$Selected_cat_d;
  // echo $sql;
...
if ($result=mysqli_query($con,$sql))
 {
  ...
  {
    $subcats[$cr]['sub_id'] = $row['sub_id']; 
    $subcats[$cr]['subcategory_name'] = $row['subcategory_name'];
    $subcats[$cr]['cat_id'] = $row['cat_id'];
